...
class TestReductions(TestCase):
    ...
    @onlyCPU
    def test_mean_out_float16_is_alias_of_return(self, device):
        a = torch.tensor([[[1.0, 1.0, 1.0, 1.0]], [[2.0, 2.0, 2.0, 2.0]], [[3.0, 3.0, 3.0, 3.0]]],
                         dtype=torch.float16, device=device)
        out = torch.empty((1, 1, 4), dtype=torch.float16, device=device)

        return_out = torch.mean(a, dim=0, keepdim=True, out=out)
        target = torch.tensor([[[2.0, 2.0, 2.0, 2.0]]], dtype=torch.float16, device=device)
